...
# Calculate centroid for each blob and write coordinates to file
def calculate_blob_coordinates(tooth_regions, mesh):
    centroids = []
    min_coords = []
    max_coords = []
    ...
    for index, colors in enumerate(tooth_regions):
        ...
        non_black_indices = [int(idx) for idx in non_black_indices]
        non_black_points = mesh.cell_centers()
        non_black_points = non_black_points.points
        non_black_points = non_black_points[non_black_indices]

        # Getting landmarks
        points_array = np.array(non_black_points)

        if points_array.size != 0:
            centroid = np.mean(points_array, axis=0)
            centroids.append(centroid)

            min_coord = np.argmin(points_array[:, 2], axis=0)
            min_coords.append(points_array[min_coord])

            max_coord = np.argmax(points_array[:, 2],  axis=0)
            max_coords.append(points_array[max_coord])  

    return centroids, min_coords, max_coords

def plot_stl_and_axes(mesh, center, x_axis, y_axis, z_axis, local_maxima, tooth_regions, addaxis, addplane, addpeaks, grid, centroids):
    # Generate Mesh
    ...
    # print(f"Contents of all_non_black_indices: {all_non_black_indices[:10]}")
    ...
    plotter.add_mesh(pv.PolyData(min_coords), color='red', point_size=20)
    plotter.add_mesh(pv.PolyData(max_coords), color='red', point_size=20)

    # Generate plot
    plotter.add_mesh(grid, color='green', line_width=10, render_lines_as_tubes=True) 
    ...
    for i, centroid in enumerate(centroids):
        plotter.add_point_labels([centroid], [f'{i+1}'], point_size=20, font_size=24, text_color='green', name=f'label_{i+1}')
    plotter.add_mesh(pv.PolyData(centroids), color='blue', point_size=20)
...
###MAIN###
def main():
    stl_filepath = 'Models\\12 year old male.stl'
    mesh = pv.read(stl_filepath)

    # ===== 3.1 FIND POSITION AND ORIENTATION =====
    ...
    tooth_regions, local_maxima = determine_regions(tooth_regions, local_maxima, perp_list, 290, max_iterations=5)
    curve, curve_points = quadratic_generator(local_maxima, center)
    perpendicular_points, perp_list, poiint_list = find_perpendicular_points(curve_points, local_maxima)
    ...
    centroids, min_coords, max_coords = calculate_blob_coordinates(tooth_regions, simplified_mesh)
    landmarks = []
    ...
    plot_stl_and_axes(simplified_mesh, center, x_axis, y_axis, z_axis, local_maxima, tooth_regions, addaxis, addplane, addpeaks, curve,landmarks)
